...
app.get("/", (req, res) => {
  res.sendFile(path.join(__dirname, "public", "socket.html")); // Express메서드로 "/" 경로로 접속한 클라이언트에게 파일을 직접 서빙
}); // 클라이언트가 접속할 때 제공할 HTML 파일 경로

io.on("connection", (socket) => {
  console.log("클라이언트 접속", socket.id); // 클라이언트가 접속했을 때 로그 출력
  ...
  conn.on("ready", () => {
    ...
    conn.shell((err, stream) => {
      ...
      socket.on("close", () => {
        ...
        socket.emit("status", "SSH 스트림 종료");
      }); // 클라이언트가 연결을 종료하면 스트림을 종료

      socket.on("error", () => {
        console.log("ssh 연결 오류" + err.message);
        socket.emit("error", "ssh 연결 오류" + err.message);
      });
